...
const App = () => {
  ...
  const CashPressed = () => {
    if (numbers.length == 0) {
      ...
      alert("Please select amount of money");
    }
    else if (randomNumber.length == 0) {
      alert("Please generate random numbers");
    }
    if (wonGame === false) {
      alert("Sorry you have lost so you are not allowed to take cash");
      ClearPressed();
      return;
    }
    else if (wonGame == true) {
      alert("Congratulations !!! You Won $" +  (parseInt(money)+parseInt(money)));
      ClearPressed();
    }
  }
  const ClearPressed = () => {
  ...
  const addMoneyPressed = (event) => {
    if (numbers.length < 5) {
      alert("You have to select 5 numbers before selecting the amount");
      return;
    }
  ...
  const numberPressed = (event) => {
    ...
    if (numbers.includes(number)) {
      ...
      if (numbers.length >= 5) {
        alert("Sorry! You cannot add more than 5 numbers");
        return;
      }
